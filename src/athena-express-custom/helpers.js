...
function startQueryExecution(config) {
    ...
    if (config.encryption) params.ResultConfiguration.EncryptionConfiguration = config.encryption
...
async function getQueryResultsFromS3(params) {
    ...
    if (params.statementType === "UTILITY" || params.statementType === "DDL") {
        ...
        const queryResults = await params.config.athena.getQueryResults(paginationParams).promise()
...
function getDataTypes() {
    return new Promise(async function (resolve) {
        const columnInfoArray = (await s3Metadata).ResultSet.ResultSetMetadata.ColumnInfo
    ...
    return new Promise(function (resolve) {
        input.pipe(
            csv({
                ...
                .on("data", (data) => {
                    cleanJson.push(addDataType(JSON.parse(data.toString("utf8")), dataTypes))
                })
                .on("finish", function () {
                    resolve(cleanJson)
                })
...
function validateConstructor(init) {
    if (!init) throw new TypeError("Config object not present in the constructor")
